(() => {
  ...
  function renderMeeting(m) {
    ...
    root.innerHTML=`<button class="back-link" data-go="/meetings">← All meetings</button>
      ...
      <section class="panel intelligence"><div class="tabs" role="tablist" aria-label="Meeting intelligence">${[["summary","Summary"],["transcript","Transcript"],["actions",`Actions ${m.actions.length}`],["highlights",`Highlights ${allHighlights(m).length}`],["ask","Ask"]].map(([id,l])=>`<button id="tab-${id}" role="tab" aria-selected="${state.tab===id}" aria-controls="tab-content" class="tab ${state.tab===id?'active':''}" data-tab="${id}">${l}</button>`).join("")}</div><div id="tab-content" class="panel-body" role="tabpanel" aria-live="polite"></div></section></div>`;
    renderTab(m);
    wireMeeting(m);
  }
  ...
  function wireMeeting(m) {
    ...
    document.querySelectorAll("[data-tab]").forEach(b=>b.onclick=()=>{state.tab=b.dataset.tab; document.querySelectorAll(".tab").forEach(x=>{x.classList.toggle("active",x===b);x.setAttribute("aria-selected",x===b);});renderTab(m);});
  ...
  function syncUI(m){const pct=state.currentTime/m.duration*100;const fill=$("#timeline-fill");if(fill)fill.style.width=`${pct}%`;const timeline=$("#timeline");if(timeline){timeline.setAttribute("aria-valuenow",Math.floor(state.currentTime));timeline.setAttribute("aria-valuetext",`${time(state.currentTime)} of ${time(m.duration)}`);}const ct=$("#current-time");if(ct)ct.textContent=time(state.currentTime);const seg=[...m.transcript].reverse().find(s=>s[0]<=state.currentTime)||m.transcript[0];const ns=$("#now-speaking");if(ns)ns.textContent=`${seg[2]} speaking`;document.querySelectorAll(".participant-tile").forEach(x=>x.classList.toggle("speaking",x.dataset.speaker===seg[2]));document.querySelectorAll(".segment").forEach(x=>x.classList.toggle("active",Number(x.dataset.start)===seg[0]));if(state.tab==="transcript"&&state.follow){const active=$(`#seg-${seg[0]}`);active?.scrollIntoView({block:"center",behavior:"smooth"});}}
  ...
  function renderShare(token){const [mid,hid]=token.split("--");const m=meetings.find(x=>x.id===mid);const h=m?allHighlights(m).find(x=>String(x[0])===hid):null;document.body.className="share-body";if(!m||!h){$("#app").innerHTML=`<main class="share-page"><div class="share-shell"><a href="#/meetings" class="share-brand"><span class="brand-mark"><span></span></span>Reverb</a><article class="share-card"><div class="share-info empty"><div class="eyebrow">Clip unavailable</div><h1>That shared moment could not be found.</h1><p class="subhead">The link may be incomplete or the highlight may have been removed.</p><a class="button primary" href="#/meetings">Browse meetings</a></div></article></div></main>`;return;}$("#app").innerHTML=`<main class="share-page"><div class="share-shell"><a href="#/meetings" class="share-brand"><span class="brand-mark"><span></span></span>Reverb</a><article class="share-card"><div class="share-video"><div><div class="eyebrow" style="color:#70d7df">Shared moment · ${time(h[3])}–${time(h[4])}</div><blockquote>“${esc(h[2])}”</blockquote><button class="button" style="margin-top:25px" id="clip-play">▶ Play ${time(h[4]-h[3])} clip</button></div></div><div class="share-info"><div class="eyebrow">${m.type} meeting</div><h1>${esc(h[1])}</h1><p class="subhead">From <strong>${esc(m.title)}</strong> · ${new Date(m.date).toLocaleDateString('en',{month:'long',day:'numeric',year:'numeric'})}</p><div class="meta">Shared by Hamza Farooq · No Reverb account required</div></div></article><p class="share-note">This is a seeded product demo. No private meeting content is used.</p></div></main>`;$("#clip-play").onclick=e=>{e.currentTarget.textContent=e.currentTarget.textContent.startsWith("▶")?"Ⅱ Pause clip":`▶ Play ${time(h[4]-h[3])} clip`;};}
  ...
  function globalAskAnswer(q){
    ...
    if(/owner|who owns|assign/.test(l)) text=allActions.slice(0,4).map(a=>`<strong>${esc(a.owner)}</strong> owns ${esc(a.title.toLowerCase())} ${cite(a.m,a.at)}`).join("; ")+".";
      ...
      const stop=["what","were","which","meetings","meeting","about","there","decide","decided","decision","decisions","did","the","and","for"];
      const keywords=l.replace(/[^a-z0-9 ]/g," ").split(" ").filter(w=>w.length>=3&&!stop.includes(w));
      const directMatches=keywords.length?allDecisions.filter(d=>keywords.some(keyword=>d.text.toLowerCase().includes(keyword))):[];
      const meetingMatches=!directMatches.length&&keywords.length?allDecisions.filter(d=>keywords.some(keyword=>d.m.title.toLowerCase().includes(keyword))):[];
      const matched=directMatches.length?directMatches:meetingMatches;
