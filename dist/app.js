(() => {
  ...
  function highlightsHTML(m) {
    return `<div class="eyebrow">Share the moment, not the hour</div><p class="subhead">Each clip preserves the transcript range and source timing.</p>${m.highlights.map(h=>`<article class="highlight-card"><div class="highlight-top"><span class="badge">${time(h[3])}–${time(h[4])}</span><span class="highlight-title">${esc(h[1])}</span></div><div class="highlight-quote">“${esc(h[2])}”</div><div class="highlight-actions"><button class="button small" data-seek="${h[3]}">▶ Play</button><button class="button small" data-share="${m.id}--${h[0]}">↗ Public clip</button></div></article>`).join("")}`;
  }
  function askHTML(m) {
